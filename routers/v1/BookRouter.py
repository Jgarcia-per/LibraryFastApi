...
async def read_book(user: user_dependency, db: db_dependency, book_id: int = Path(ge=1)):
    """
...
async def delete_book(user: user_dependency, db: db_dependency, book_id: int = Path(ge=1)):
    """
    Delete book by id
    """
    if user is None:
